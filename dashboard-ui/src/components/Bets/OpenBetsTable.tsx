...
type OpenBetsTableProps = {
  bets: OpenBet[]
}

export function OpenBetsTable({ bets }: OpenBetsTableProps) {
  return (
    <div className="cyber-card overflow-hidden p-4">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          <h3 className="font-orbitron text-lg text-cyber-text">Open Bets</h3>
          <p className="text-sm text-cyber-muted">{bets.length} active positions</p>
        </div>
      </div>
          ...
          <tbody>
            {bets.map((bet) => (
              <tr key={bet.id} className="border-b border-cyber-border/60 text-cyber-text">
                ...
                <td className="py-3 pr-4">{formatNumber(bet.current_price)}</td>
                <td className="py-3 pr-4">{formatNumber(bet.shares)}</td>
                <td className="py-3 pr-4">{formatCurrency(bet.cost)}</td>
                <td className={`py-3 pr-4 ${bet.pnl_unrealized >= 0 ? 'text-cyber-green' : 'text-cyber-red'}`}>
                  {formatCurrency(bet.pnl_unrealized)}
                </td>
                <td className="py-3 text-cyber-muted">{formatDate(bet.placed_at)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      {bets.length === 0 ? <p className="py-8 text-center text-sm text-cyber-muted">No open bets</p> : null}
    </div>
  )
}
